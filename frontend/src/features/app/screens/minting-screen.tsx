...
const PUNS = [
  ...
  'Sprinkling on the final toppings…',
  'Checking the crust is golden…',
  'Your pizza is looking crispy…',
];

export function MintingScreen({ payment, tokenURI, onHome, isMintPending, mintError }: MintingScreenProps) {
  const [punIndex, setPunIndex] = useState(0);
  const [progress, setProgress] = useState(15);
  const errorFlagged = useRef(false);

  const isError = !!mintError;

  // Flag error once
  useEffect(() => {
    if (mintError && !errorFlagged.current) {
      errorFlagged.current = true;
    }
  }, [mintError]);
  ...
  useEffect(() => {
    if (isError) return;
    const target = isMintPending ? 85 : 50;
    const id = setInterval(() => {
      setProgress(p => {
  ...
  return (
    <AppShell onHome={isError ? onHome : undefined}>
      ...
      <div style={{
        ...
        border: `3px solid ${isError ? C.red + '60' : C.orange + '50'}`,
        padding: '28px 16px 24px',
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16,
        boxShadow: isError ? `4px 4px 0 ${C.red}20` : `4px 4px 0 ${C.orangeD}25`,
        position: 'relative', overflow: 'hidden',
      }}>
        <div style={{
          fontSize: 90,
          animation: isError ? undefined : 'float-pizza 3.5s ease-in-out infinite',
        ...
        <div style={{ textAlign: 'center' }}>
          <p style={{
            ...
            color: isError ? C.red : C.orangeD, margin: '0 0 6px',
          }}>
            {headline}
          </p>
      ...
      {!isError && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <p style={{ fontFamily: F.display, fontSize: 14, color: C.orange, margin: 0 }}>{progress}%</p>
          </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {([
          ...
          { label: `Minting on Base (${payment === 'usdc' ? 'USDC' : 'ETH'})`,   done: false, active: !!isMintPending },
          { label: 'Confirming on-chain…',                                        done: false, active: !isMintPending && !isError },
      ...
      {isError && (
        <div style={{
          ...
          <div>
            <p style={{ fontFamily: F.body, fontWeight: 800, color: C.red, margin: '0 0 2px', fontSize: 13 }}>
              {mintError}
            </p>
